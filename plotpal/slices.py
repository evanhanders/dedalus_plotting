...
from plotpal.plot_grid import PlotGrid, RegularColorbarPlotGrid
...
class Colormesh:
    ...
    vector_ind: Optional[int] = (
        None  # If not None, plot the vector component with this index. For use with d3 vector fields.
    )
    x_basis: str = (
        "x"  # The dedalus basis name that the profile spans in the horizontal-direction of the plot
    )
    y_basis: str = (
    ...
    remove_x_mean: bool = (
        False  # If True, remove the mean value over the axis plotted in the x- direction
    )
    divide_x_std: bool = False  # If True, divide by the stdev over the x- direction
    pos_def: bool = (
        False  # If True, field is positive definite and colormap should span from zero to max.
    )
    ...
    cmap_exclusion: float = (
        0.005  # The fraction of the colormap to exclude from the min/max values
    )
    linked_cbar_cm: Optional["Colormesh"] = (
        None  # A Colormesh object that this object shares a colorbar with
    )
    linked_profile_cm: Optional["Colormesh"] = (
        None  # A Colormesh object that this object shares a mean profile with
    )
    transpose: bool = (
        False  # If True, transpose the colormap when plotting; useful when x_basis has an index after y_basis in dedalus data.
    )
    ...
    color_plot: Optional[matplotlib.collections.QuadMesh] = (
        None  # The pcolormesh object for the plot
    )
    ...
    def _modify_field(self, field: np.ndarray) -> np.ndarray:
        ...
        if self.linked_profile_cm is not None:
            ...
            if self.divide_x_std:
                ...
                assert isinstance(
                    self.divided_std, np.ndarray
                ), "divide_x_std only works for 2D fields"
    ...
    def _get_pcolormesh_coordinates(
        self, dset: Union[h5py.Dataset, RolledDset]
    ) -> None:
    ...
    def _setup_colorbar(
        ...
        ), "cb.solids must be a matplotlib.collections.QuadMesh"
            ...
            if self.label is None:
                if self.vector_ind is not None:
                    cax.text(
                        1.05,
                        0.5,
                        "{:s}[{}]".format(self.task, self.vector_ind),
                        transform=cax.transAxes,
                        va="center",
                        ha="left",
                    )
    ...
    def plot_colormesh(
        ...
        if self.transpose:
            field = field.T
        assert (
            self.xx is not None and self.yy is not None
        ), "xx and yy must be set before plotting"
        self.color_plot = ax.pcolormesh(
            self.xx,
            self.yy,
            field.real,
            cmap=self.cmap,
...
class OrthographicColormesh(Colormesh):
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "Cartopy must be installed for plotpal Orthographic plots"
            )
...
class SlicePlotter(SingleTypeReader):
    ...
    def add_meridional_colormesh(self, left_task: str, right_task: str, **kwargs) -> None:  # type: ignore
        ...
        self.colormeshes.append(
            (self.counter, MeridionalColormesh(left_task, left=True, **kwargs))
        )
        self.colormeshes.append(
            (
                self.counter,
                MeridionalColormesh(
                    right_task,
                    linked_cbar_cm=self.colormeshes[-1][1],
                    linked_profile_cm=self.colormeshes[-1][1],
                    **kwargs,
                ),
            )
        )
    ...
    def add_ball_shell_polar_colormesh(  # type: ignore
        self, ball_task: str, shell_task: str, r_stitch: float, r_outer: float, **kwargs
    ) -> None:
        ...
        self.colormeshes.append(
            (
                self.counter,
                PolarColormesh(ball_task, r_inner=0.0, r_outer=r_stitch, **kwargs),
            )
        )
        self.colormeshes.append(
            (
        ...
        **kwargs,
        ...
        self.colormeshes.append(
            (
                self.counter,
                MeridionalColormesh(
                    ball_left_task, left=True, r_inner=0.0, r_outer=r_stitch, **kwargs
                ),
            )
        )
        ...
        self.colormeshes.append(
            (
                ...
                MeridionalColormesh(
                    ...
                    r_inner=0.0,
                    r_outer=r_stitch,
                    linked_cbar_cm=first_cm,
                    linked_profile_cm=first_cm,
                    **kwargs,
                ),
            )
        )
    ...
    def add_ball_2shells_polar_colormesh(  # type: ignore
        ...
        **kwargs,
        ...
        self.colormeshes.append(
            (
                self.counter,
                PolarColormesh(fields[0], r_inner=0, r_outer=r_stitches[0], **kwargs),
            )
        )
        self.colormeshes.append(
            (
                ...
                PolarColormesh(
                    fields[1],
                    r_inner=r_stitches[0],
                    r_outer=r_stitches[1],
                    linked_cbar_cm=self.colormeshes[-1][1],
                    **kwargs,
                ),
            )
        ...
        self.colormeshes.append(
            (
                ...
                PolarColormesh(
                    fields[2],
                    r_inner=r_stitches[1],
                    r_outer=r_outer,
                    linked_cbar_cm=self.colormeshes[-2][1],
                    **kwargs,
                ),
            )
        )
    ...
    def plot_colormeshes(
        self, start_fig: int = 1, dpi: int = 200, mpl_kwargs: dict = {}
    ) -> None:
        """
        ...
        with self.my_sync:
            ...
            while self.writes_remain():
                ...
                ), "current_file_handle must be set"
                ...
                self.grid.fig.savefig(
                    "{:s}/{:s}_{:06d}.png".format(
                        self.out_dir, self.out_name, int(write_num + start_fig - 1)
                    ),
                    dpi=dpi,
                    bbox_inches="tight",
                )
