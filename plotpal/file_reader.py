...
class FileReader:
    ...
    def __init__(
            ...
            sub_dirs: list[str] = ['slices',], 
            num_files: list[Optional[int]] = [None,],
            start_file: int = 1, 
        ...
        self.file_starts: dict[str, np.ndarray] = OrderedDict() # Start index of files for each file type
        self.file_counts: dict[str, np.ndarray] = OrderedDict() # Number of counts per file for each file type
        self.comms: dict[str, MPI.Comm] = OrderedDict() # MPI communicators for each file type
        ...
        for k, files in self.file_lists.items():
            ...
            if num_procs == self.global_comm.size:
                ...
                if self.global_comm.rank < num_procs:
                    self.comms[k] = self.global_comm.Create(self.global_comm.Get_group().Incl(range(num_procs)))
                else:
...
class SingleTypeReader():
    ...
    def writes_remain(self) -> bool:
        ...
        """
        if not self.idle:
            if self.current_write >= self.writes - 1 and self.current_file_handle is not None:
                ...
                elif self.current_file_number != next_file_number and self.current_file_handle is not None:
                    self.current_file_handle.close()
                    self.current_file_number = next_file_number
                    self.current_file_name = self.files[self.current_file_number]
                    self.current_file_handle = h5py.File(self.current_file_name, 'r')
    ...
    def get_dsets(
        ...
        if not self.idle:
            ...
            assert self.current_file_handle is not None, ValueError("No file handle found.")

            f = self.current_file_handle    
            ni = self.file_index[self.current_write]
            for k in tasks:
                if isinstance(self.reader, RollingFileReader):
                    assert self.roll_counts is not None, ValueError("No roll counts found.")
                    assert self.roll_starts is not None, ValueError("No roll starts found.")
