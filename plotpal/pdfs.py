    ...
    def _get_interpolated_slices(
        ...
        for k in dsets.keys():
            ...
            interp = RegularGridInterpolator(
                (x.flatten(), y.flatten()), dsets[k][ni], method="linear"
            )
    ...
    def _get_interpolated_volumes(
        ...
        for k in dsets.keys():
            ...
            for i in range(file_data[k].shape[0]):
                if self.comm.rank == 0:
                    print(
                        "interpolating {} ({}/{})...".format(
                            k, i + 1, file_data[k].shape[0]
                        )
                    )
                    stdout.flush()
    ...
    def calculate_pdfs(
        ...
        with self.my_sync:
            ...
            while self.writes_remain():
                ...
                if threeD:
                    file_data = self._get_interpolated_volumes(
                        dsets, ni, uneven_basis=uneven_basis
                    )
                else:
                    file_data = self._get_interpolated_slices(
                        dsets, ni, uneven_basis=uneven_basis
                    )

                # Create histograms of data
                for field in pdf_list:
                    hist, bin_vals = np.histogram(
                        file_data[field], bins=bins, range=tuple(bounds[field])
                    )
    ...
    def plot_pdfs(
        ...
        with self.my_sync:
            ...
            for k, data in self.pdfs.items():
                ...
                title = r"$\mu$ = {:.2g}, $\sigma$ = {:.2g}, skew = {:.2g}, kurt = {:.2g}".format(
                ...
                grid.fig.savefig(
                    "{:s}/{:s}_pdf.png".format(self.out_dir, k),
                    dpi=dpi,
                    bbox_inches="tight",
                )
                ax.clear()
                ...
                for k, data in self.pdfs.items():
                    ...
                    for d, n in ((pdf, "pdf"), (xs, "xs")):
                        this_group.create_dataset(
                            name=n, shape=d.shape, dtype=np.float64
                        )
                        f["{:s}/{:s}".format(k, n)][:] = d
