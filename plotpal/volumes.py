...
from plotpal.plot_grid import PlotGrid, PyVista3DPlotGrid, RegularColorbarPlotGrid

logger = logging.getLogger(__name__.split(".")[-1])


def build_s2_vertices(
...
def build_spherical_vertices(
    phi: np.ndarray, theta: np.ndarray, r: np.ndarray, Ri: float, Ro: float
...
def spherical_to_cartesian(
    phi: np.ndarray, theta: np.ndarray, r: np.ndarray, mesh: bool = True
) -> np.ndarray:
...
class Box:
    ...
    left_mid: Optional[str] = (
        None  # The name of the field to plot on the left side of the cutout
    )
    right_mid: Optional[str] = (
        None  # The name of the field to plot on the right side of the cutout
    )
    top_mid: Optional[str] = (
        None  # The name of the field to plot on the top side of the cutout
    )
    ...
    vector_ind: Optional[int] = (
        None  # The index of the component to plot if the field is a vector field
    )
    ...
    cmap_exclusion: float = (
        0.005  # The fraction of the colormap to exclude from the minimum and maximum values
    )
    ...
    remove_mean: bool = (
        False  # If True, the mean of the field is subtracted from the field before plotting
    )
    remove_x_mean: bool = (
        False  # If True, the mean of the field in the x-direction is subtracted from the field before plotting
    )
    divide_x_std: bool = (
        False  # If True, the field is divided by the standard deviation in the x-direction before plotting
    )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.left_mid is not None
            and self.right_mid is not None
            and self.top_mid is not None
        ):
            self.cutout = True
    ...
    def _get_minmax(self, field: np.ndarray) -> tuple[float, float]:
        ...
        if self.pos_def:
            vals = np.sort(vals)
            if np.mean(vals) < 0:
                vmin, vmax = vals[int(self.cmap_exclusion * len(vals))], 0.0
            else:
                vmin, vmax = 0.0, vals[int((1 - self.cmap_exclusion) * len(vals))]
        else:
    ...
    def _setup_colorbar(
        ...
        assert (
            cb.solids is not None
        ), "ColorbarBase has no attribute 'solids'; something went wrong instantiating cbar."
        cb.solids.set_rasterized(True)
        cb.set_ticks(())
        cax.text(
            -0.01,
            0.5,
            r"$_{{{:.2e}}}^{{{:.2e}}}$".format(vmin, vmax),
    ...
    def plot_colormesh(
        ...
        if self.cutout:
            assert (
                ...
                and self.right_mid is not None
                and self.top_mid is not None
            ), "left_mid, right_mid, and top_mid must be specified for a cutout box."
            ...
            xy_side = construct_surface_dict(
                self.x,
                self.y,
                self.z_max,
                top_field,
                x_bounds=(self.x_min, self.x_mid + side_stretch * self.Lx),
                y_bounds=(self.y_min, self.y_mid + side_stretch * self.Ly),
            )
            xz_side = construct_surface_dict(
                self.x,
                self.y_max,
                self.z,
                right_field,
                x_bounds=(self.x_min, self.x_mid + side_stretch * self.Lx),
                z_bounds=(self.z_min, self.z_mid + side_stretch * self.Lz),
            )
            yz_side = construct_surface_dict(
                self.x_max,
                self.y,
                self.z,
                left_field,
            ...
            xy_mid = construct_surface_dict(
                self.x,
                self.y,
                self.z_mid,
                mid_top_field,
                x_bounds=(self.x_mid + mid_stretch * self.Lx, self.x_max),
                y_bounds=(self.y_mid + mid_stretch * self.Ly, self.y_max),
                bool_function=np.logical_and,
            )
            xz_mid = construct_surface_dict(
                self.x,
                self.y_mid,
                self.z,
                mid_right_field,
                x_bounds=(self.x_mid + mid_stretch * self.Lx, self.x_max),
                z_bounds=(self.z_mid + mid_stretch * self.Lz, self.z_max),
                bool_function=np.logical_and,
            )
            yz_mid = construct_surface_dict(
                self.x_mid,
                self.y,
                self.z,
                mid_left_field,
        ...
        vmin, vmax = self._get_minmax(
            left_field
        )  # Could use any field; should use all of them.
        ...
        for i, d in enumerate(side_list):
            ...
            if engine == "matplotlib":
                assert (
                    ax is not None
                ), "An Axes3D object must be passed to plot the box in matplotlib."
                ...
                ax.plot_wireframe(
                    x, y, z, ccount=1, rcount=1, linewidth=1, color="black"
                )
            elif engine == "pyvista":
                assert (
                    pl is not None
                ), "A pyvista plotter object must be passed to plot the box in pyvista."
                ...
                if self.first:
                    ...
                    if i == 0:
                        try:
                            ...
                            raise ImportError(
                                "PyVista must be installed for 3D pyvista plotting in plotpal"
                            )
        ...
        if engine == "matplotlib":
            ...
            assert (
                cax is not None
            ), "An Axes object must be passed to plot the colorbar in matplotlib."
            if self.cutout:
                ax.plot_wireframe(
                    x_a, y_a, z_a, ccount=1, rcount=1, linewidth=1, color="black"
                )
                ax.plot_wireframe(
                    x_b, y_b, z_b, ccount=1, rcount=1, linewidth=1, color="black"
                )
...
class CutSphere:
    ...
    inner_shell: Optional[str] = (
        None  # The name of the field to plot on the inner shell
    )
    ...
    remove_radial_mean: bool = (
        False  # If True, the (phi,theta) average of the field is subtracted from the field before plotting
    )
    divide_radial_stdev: bool = (
        False  # If True, the field is divided by the standard deviation in (phi,theta) before plotting
    )

    def __post_init__(self) -> None:
        self.first = True
        self.radial_mean: Optional[np.ndarray] = (
            None  # will be computed in the equatorial slice
        )
        self.radial_stdev: Optional[np.ndarray] = (
            None  # will be computed in the equatorial slice
        )
    ...
    def plot_colormesh(
        ...
        if self.first:
            ...
            phi_vert, theta_vert, r_vert = build_spherical_vertices(
                self.phi, self.theta, self.r, self.r_inner, self.r_outer
            )
            phi_vert_out, theta_vert_out, r_vert_out = build_spherical_vertices(
                self.shell_phi, self.shell_theta, self.r, self.r_inner, self.r_outer
            )
            phi_vert_in, theta_vert_in, r_vert_in = build_spherical_vertices(
                self.shell_phi, self.shell_theta, self.r, self.r_inner, self.r_outer
            )
            theta_mer = np.concatenate([-self.theta, self.theta[::-1]])
            self.x_out, self.y_out, self.z_out = spherical_to_cartesian(
                phi_vert_out,
                theta_vert_out,
                np.array(
                    [
            ...
            self.x_in, self.y_in, self.z_in = spherical_to_cartesian(
                phi_vert_in,
                theta_vert_in,
                np.array(
                    [
            ...
            self.x_eq, self.y_eq, self.z_eq = spherical_to_cartesian(
                phi_vert,
                np.array(
                    [
                        np.pi / 2,
            ...
            self.x_mer, self.y_mer, self.z_mer = spherical_to_cartesian(
                np.array(
                    [
                        phi_mer1,
                    ]
            ...
            self.in_data["x"], self.in_data["y"], self.in_data["z"] = (
                self.x_in,
                self.y_in,
                self.z_in,
            )
            self.out_data["x"], self.out_data["y"], self.out_data["z"] = (
                self.x_out,
                self.y_out,
                self.z_out,
            )
            self.eq_data["x"], self.eq_data["y"], self.eq_data["z"] = (
                self.x_eq,
                self.y_eq,
                self.z_eq,
            )
            self.mer_data["x"], self.mer_data["y"], self.mer_data["z"] = (
                self.x_mer,
                self.y_mer,
                self.z_mer,
            )
        ...
        self.radial_mean_func = interp1d(
            self.r_full,
            self.radial_mean.squeeze(),
            kind="linear",
            bounds_error=False,
            fill_value="extrapolate",
        )
        self.radial_stdev_func = interp1d(
            self.r_full,
            self.radial_stdev.squeeze(),
            kind="linear",
            bounds_error=False,
            fill_value="extrapolate",
        )
        ...
        self.radial_stdev[0, : int(num_r * 0.05)] = np.linspace(
            mean_interior, mean_boundary, int(num_r * 0.05)
        )
        eq_field = self._modify_field(eq_field)
        self.eq_data["field"] = np.pad(
            eq_field.squeeze()[:, self.r_full <= self.r_outer],
            ((1, 0), (1, 0)),
            mode="edge",
        )
        ...
        for i, d in enumerate(self.data_dicts):
            ...
            if self.first:
                ...
                d["clip"][label] = d["grid"].clip_scalar(
                    "mask", invert=False, value=0.5
                )[label]
...
class BoxPlotter(SingleTypeReader):
    ...
    def _groom_grid(self) -> tuple[list[Axes3D], list[matplotlib.axes.Axes]]:
        ...
        assert isinstance(
            self.grid, PlotGrid
        ), "The plot grid must be initialized before plotting."
        axs, caxs = [], []
    ...
    def plot_boxes(
        self, start_fig: int = 1, dpi: int = 200, extra_kwargs: dict = {}
    ) -> None:
        """
    ...
    def plot_boxes(self, start_fig: int = 1, extra_kwargs: dict = {}) -> None:
        ...
        with self.my_sync:
            ...
            while self.writes_remain():
                dsets, ni = self.get_dsets(tasks)
                assert isinstance(
                    self.current_file_handle, h5py.File
                ), "The current file handle must be an h5py file."
...
class PyVistaSpherePlotter(SingleTypeReader):
    ...
    def plot_spheres(
        self, start_fig: int = 1, plot_colormesh_kwargs: dict = {}
    ) -> None:
        """
        Plot 3D renderings of 2D dedalus data slices at each timestep.
        """
        ...
        with self.my_sync:
            ...
            for k, sp in self.spheres:
                tasks = (
                    sp.equator + sp.left_meridian + sp.right_meridian + [sp.outer_shell]
                )
            ...
            while self.writes_remain():
                ...
                for k, sp in self.spheres:
                    self.grid.change_focus_single(k)
                    sp.plot_colormesh(
                        dsets, ni, pl=self.grid.pl, **plot_colormesh_kwargs
                    )
