...
class PlotGrid:
    ...
    def __init__(
        self, col_inch: float = 3, row_inch: float = 3, pad_factor: float = 10
    ):
        """
    ...
    def add_axis(
        ...
        if sum([polar, mollweide, orthographic, threeD]) > 1:
            raise ValueError(
                "Only one of polar, mollweide, orthographic, or threeD can be True."
            )
    ...
    def make_subplots(self) -> None:
        ...
        for spec in self.specs:
            ...
            if col_ceil > self.ncols:
                self.ncols = col_ceil
        self.fig = plt.figure(
            figsize=(self.ncols * self.col_inch, self.nrows * self.row_inch)
        )
        ...
        for spec in self.specs:
            ...
            if spec["cbar"]:
                ...
                if spec["kwargs"]["polar"]:
                    ...
                    cbar_delta_x = 0.15 * delta_x
                    delta_y *= 0.85
                self.cbar_axes["ax_{}-{}".format(spec["row_num"], spec["col_num"])] = (
                    self.fig.add_axes(
                        (cbar_x_anchor, cbar_y_anchor, cbar_delta_x, cbar_delta_y)
                    )
                )
            self.axes["ax_{}-{}".format(spec["row_num"], spec["col_num"])] = (
                self.fig.add_axes(
                    (x_anchor, y_anchor, delta_x, delta_y), **spec["kwargs"]
                )
            )
