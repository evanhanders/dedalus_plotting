...
from plotpal.file_reader import RolledDset, SingleTypeReader
...
class ScalarFigure(RegularPlotGrid):
    ...
    def add_field(
        self, panel: int, field: str, log: bool = False, extra_kwargs: dict = {}
    ) -> None:
        """
    ...
    def _read_fields(self) -> None:
        ...
        with self.my_sync:
            ...
            while self.writes_remain():
                ...
                ), "current_file_handle is not an h5py.File"
                ...
                for f in self.fields:
                    ...
                    if rdsets is not None and ri is not None:
                        trace_data["rolled_{}".format(f)].append(
                            rdsets[f][ri].squeeze()
                        )

                trace_data["sim_time"].append(
                    self.current_file_handle["scales"]["sim_time"][ni]
                )

            for f in self.fields:
                self.trace_data[f] = np.array(trace_data[f])
    ...
    def plot_figures(self, dpi: int = 200, fig_name: str = "output") -> None:
        ...
        with self.my_sync:
            ...
            for j, fig in enumerate(self.figures):
                for i, k in enumerate(fig.panels):
                    ...
                    for fd, log, kwargs in fig.panel_fields[i]:
                        ...
                        if self.rolled_reader is not None:
                            ...
                            ax.plot(
                                self.trace_data["sim_time"],
                                self.trace_data["rolled_{}".format(fd)],
                                label="rolled_{}".format(fd),
                                **kwargs,
                            )
                        if log:
                            ax.set_yscale("log")
                    ax.set_xlim(
                        self.trace_data["sim_time"].min(),
                        self.trace_data["sim_time"].max(),
                    )
                ...
                fig.fig.savefig(
                    "{:s}/{:s}.png".format(self.out_dir, fig_name),
                    dpi=dpi,
                    bbox_inches="tight",
                )
            self._save_traces()

    def plot_convergence_figures(
        self, dpi: int = 200, fig_name: str = "output"
    ) -> None:
        """
        ...
        with self.my_sync:
            ...
            for j, fig in enumerate(self.figures):
                for i, k in enumerate(fig.panels):
                    ...
                    for fd, log, kwargs in fig.panel_fields[i]:
                        ...
                        if "rolled_{}".format(fd) in self.trace_data:
                            ...
                            these_kwargs["label"] = (
                                "1 - ({:s})/(final rolled mean)".format(fd)
                            )
                        else:
                            final_mean = np.mean(
                                self.trace_data[fd][
                                    -int(0.1 * len(self.trace_data[fd])) :
                                ]
                            )
                            these_kwargs["label"] = "1 - ({:s})/(last 10% mean)".format(
                                fd
                            )
                        ax.plot(
                            self.trace_data["sim_time"],
                            np.abs(1 - self.trace_data[fd] / final_mean),
                            **these_kwargs,
                        )
                    ax.set_yscale("log")
                    ax.set_xlim(
                ...
                fig.fig.savefig(
                    "{:s}/{:s}_convergence.png".format(self.out_dir, fig_name),
                    dpi=dpi,
                    bbox_inches="tight",
                )
