...
shell = d3.ShellBasis(
    coords, shape=(Nphi, Ntheta, Nr), radii=(Ri, Ro), dealias=dealias, dtype=dtype
)
...
slices.add_task(b(r=(Ri + Ro) / 2), scales=dealias, name="bmid")
slices.add_task(flux(r=Ro), scales=dealias, name="flux_r_outer")
slices.add_task(flux(r=Ri), scales=dealias, name="flux_r_inner")
slices.add_task(flux(phi=0), scales=dealias, name="flux_phi_start")
slices.add_task(flux(phi=3 * np.pi / 2), scales=dealias, name="flux_phi_end")
slices.add_task(flux(theta=np.pi / 2), scales=dealias, name="flux(equator)")
