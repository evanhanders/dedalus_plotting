...
See: https://github.com/DedalusProject/dedalus/tree/master/examples/ivp_2d_rayleigh_benard
...
To run and plot using e.g. 4 processes and plotpal scripts:
...
profiles.add_task(plane_avg(ez @ (u * b)), name="conv_flux")
profiles.add_task(plane_avg(ez @ (-kappa * d3.grad(b))), name="cond_flux")

vol_avg = lambda A: d3.Integrate(d3.Integrate(A, coords["x"]), coords["z"])
scalars = solver.evaluator.add_file_handler("scalars", sim_dt=0.1, max_writes=1e6)
scalars.add_task(vol_avg(np.sqrt(u @ u) / nu), name="Re")
scalars.add_task(
    1 + vol_avg(ez @ (u * b)) / vol_avg(ez @ (-kappa * d3.grad(b))), name="Nu"
)
